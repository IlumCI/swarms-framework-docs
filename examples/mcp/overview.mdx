...
<CardGroup cols={2}>
  ...
  <Card title="Serve an agent over MCP" icon="rocket" href="/examples/mcp/mcp-deployer-serve-agent">
    Turn an agent into an MCP server with `MCPDeployer`, behind an API key, and call it from a second agent.
    <br />**Model:** `claude-sonnet-5`
  </Card>
  <Card title="Serve a team from one server" icon="users" href="/examples/mcp/mcp-deployer-serve-team">
    Two agents, a workflow, and a function as four tools on one `MCPDeployer` server.
    <br />**Model:** `claude-sonnet-5`
  </Card>
